...
or cached version — always read the live file from the configured DoD path (set via `designs.dod_path` in config, or the default DoD document in the designs directory).
...
**Section 1 — Issue completeness**
Confirm the issue tracker ticket (key provided in the sprint input) contains:
- Context (why we are doing this)
- Requirements (functional and non-functional)
- Acceptance criteria (testable bullets or checklist)
- Design references (links to ADD or ADR docs)
...
Use the `bdd_runner` tool to actually run the BDD scenarios tagged with the issue key
(e.g. `tags="@<issue-key>"`). Include the runner output verbatim in this report as evidence.
"Scenarios were specified" is NOT sufficient — tests must be implemented and pass.

Confirm:
- BDD scenarios exist, are annotated with the issue key and category tags, and cover 100% of ACs
...
- A test report summary is available for an issue tracker comment

**Section 3 — Pull request format**
Confirm:
- One primary PR per story
- PR title includes the issue key (e.g., `feat(<scope>): … <issue-key>`)
- PR description links the issue tracker ticket and design docs

**Section 4 — Issue tracker ↔ VCS linking**
Confirm:
- Branch name includes the issue key
- Commits include the issue key (smart-commit format or equivalent)
- PR is linked in the issue tracker (or flag for admin to verify if tool cannot confirm)
...
If this work completes a planned delivery phase: confirm staging has been promoted or flag it as required.
